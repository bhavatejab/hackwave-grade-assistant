...
export const UploadWorkflowPage: React.FC = () => {
  ...
  const [, setRubricSelected] = useState<File | null>(null)
  const [isEvaluating, setIsEvaluating] = useState(false)
  const [errorMessage, setErrorMessage] = useState<string | null>(null)

  // Direct question, rubric, and student answer inputs (compatible with OCR)
  ...
  const handleEvaluate = async () => {
    ...
    if (!studentAnswer.trim()) {
      setErrorMessage("Student's answer cannot be empty. Enter answer text or provide an OCR scan.")
      return
    }
    ...
    for (const c of rubricCriteria) {
      if (!c.description.trim()) {
        setErrorMessage('All rubric criteria must have a non-empty description.')
        return
      }
      if (isNaN(c.max_score) || c.max_score <= 0) {
        setErrorMessage('Rubric criterion max score must be greater than 0.')
        return
      }
    }
    ...
    try {
      // Connect directly to FastAPI backend POST /api/grade
      ...
      // Update EvaluationContext with real backend grading response
      ...
      addNotification({
        ...
        message: `Evaluation received from backend: ${result.total_score}/${result.max_score} marks awarded.`,
        type: 'success',
      })
